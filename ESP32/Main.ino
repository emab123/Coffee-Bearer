...
#include "SPIFFS.h" // <-- 1. ADICIONADO: Biblioteca do Sistema de Arquivos
...
void configurar_rotas_api() {
    ...
    // Rota principal (/) agora serve o arquivo index.html do SPIFFS
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
        request->send(SPIFFS, "/index.html", "text/html");
    });

    // Rota para servir o arquivo de estilo (style.css)
    server.on("/style.css", HTTP_GET, [](AsyncWebServerRequest *request){
        request->send(SPIFFS, "/style.css", "text/css");
    });

    // Rota para servir o arquivo de script (script.js)
    server.on("/script.js", HTTP_GET, [](AsyncWebServerRequest *request){
        request->send(SPIFFS, "/script.js", "text/javascript");
    });

    // ---- SUAS ROTAS DE API EXISTENTES (PERMANECEM IGUAIS) ----
